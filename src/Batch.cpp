...
{
	for (int i = 0; i < dimensions.size(); i++) assert(dimensions.begin()[i] > 0);  m_Dimensions[i] = dimensions.begin()[i];
	// allocate data memory ; 
	m_Data = new float[m_Dimensions[0] * m_Dimensions[1] * m_Dimensions[2] * m_Dimensions[3]]; 
}
